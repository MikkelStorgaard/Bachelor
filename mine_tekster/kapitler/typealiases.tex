...
\begin{prooftree}
  \AxiomC{$\Gamma \vdash typedecl \Rightarrow \Gamma ' $}
    \UnaryInfC{$\Gamma \vdash$ \texttt{type} \textit{strid }\texttt{=}
      \textit{Type} $\to$ $\{ strid \to Type \} \oplus \Gamma \Rightarrow
      \Gamma'
      \}$}
\end{prooftree}
where
...
\begin{prooftree}
  \AxiomC{$\Gamma \vdash typedecls \Rightarrow \Gamma ' $}
  \AxiomC{Strids = $strid_1 , ... , strid_n$ }
    \BinaryInfC{ $\Gamma \vdash$ \texttt{type} \textit{Strids}\texttt{=}
      \textit{Type} $\to$ $\{ strid_1 \to Type, .. , strid_n \to Type \} \oplus \Gamma \Rightarrow \Gamma' \}$}
\end{prooftree}
